...
void pick_instruction(FILE *out_file, FILE *string_file, quad_list_item *curr_quad);

// Prints assembly for block jump
void pick_jump_instruction(FILE *out_file, basic_block *block);

// Gets alignment of variable
int get_alignment_of(union astnode *node);

// Given node, returns assembly reference
char *node_to_assembly(union astnode *node);
...
union astnode *allocate_register(union astnode *node);

// Frees register used by node
void free_register(union astnode *node); */
